...
public class SafeWalkThroughGrid{
    public static void main(String[] args) {
        ...
        list.add(new ArrayList<>(List.of(0,1,0,0,0)));
        list.add(new ArrayList<>(List.of(0,1,0,1,0)));
        list.add(new ArrayList<>(List.of(0,0,0,1,0)));
        System.out.println(findSafeWalk(list,1));
    }
    static class Pair{
        int row;
        int col;
    ...
    public static boolean findSafeWalk(List<List<Integer>> grid, int health) {
        ...
        while(!queue.isEmpty()){
            Pair cur=queue.poll();
            if(cur.row==n-1&&cur.col==m-1)return true;
            for(int i=0;i<4;i++){
